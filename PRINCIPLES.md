...
| G3-M | P3/P6/P10 | Claude Code 项目级机器闸门由共享设置与 `validation/system/governance_gate.mjs`共同实施：G3 禁读边界向一切派生角色继承；无点名范围的检索、裸递归检索、后台任务及临时探针仓创建在工具执行前 fail-closed；门本身由系统测试入口持续自检。 |
...
2. 共享 Claude Code 设置默认禁用派生角色，并以 deny 规则和 `PreToolUse` command hook 在工具执行前阻断 G3 边界、裸递归检索、后台任务及自建临时探针仓。
...
- 2026-07-18 v1.1 登记 Part 9 执行边界事件；强化 G6/G10，新增 G3-M 机器闸门与销案条件（起草：评审方；待项目决策人批准）。
