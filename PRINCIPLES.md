...
| G3-M | P3/P6/P10 | Claude Code 项目级机器闸门由共享设置与 `validation/system/governance_gate.mjs` 共同实施：G3 禁读边界向一切派生角色继承；无点名范围的检索、裸递归检索、后台任务及临时探针仓创建在工具执行前 fail-closed；`Monitor` 与独立 `PowerShell` 工具默认禁用，防止绕过受控 Bash 面；**获批脚本只能以其登记真实路径与全局登记模式集内的单个字面模式实参调用，缺参数、未知模式、变量化或拼接构造的路径与模式、通配路径、同名异路径及多余参数一律在工具执行前拒绝——该门只强制全局登记模式集，当前工作包更窄的范围仍由文本授权与停报纪律约束**；门本身由系统测试入口持续自检。 |
...
公开事实摘要：一个只读的制包会话在设计一项扫描步骤时，需要确认公共侧禁现扫描工具是否支持某种调用形态，遂以数个未被当次授权点名的模式实参试探性调用了该工具。三次调用均为只读，未改动任何文件、未产生任何对象、未触及 G3 边界，工作树全程保持干净；越界由该会话自身的事后自检发现并主动上报。所有者裁定不予追溯授权。完整事实、当次授权原文与所有者裁决留在私有事实记录。

根因与永久化处理：

1. 根因不是疏忽而是范围替换——会话以自己推测的授权意图替代了授权的书面范围，并把「风险低」误当作「已获授权」。风险高低与是否获得授权是两条互相独立的轴，只有决策人能决定哪些低风险动作可做。该动作亦非必需：存在完全合规的替代路径，即把待确认事项直接立项交决策人裁定。
2. 该边界此前纯靠会话自律，不在任何机器门覆盖内。既有拦截面针对的是「禁用某个工具或某种形态」，而本次是「用一个获批的工具做授权范围外的事」，属整类未覆盖。
3. `validation/system/governance_gate.mjs` 增设获批脚本参数面有限命令门：调用必须绑定该脚本的规范化登记路径，并只接受全局登记模式集内的单个字面模式实参；缺参数、未知模式、变量化或拼接构造的路径与模式、通配路径、同名异路径与多余参数在工具执行前 fail-closed 拒绝。其诊断性列举模式一并不予放行——该模式会把命中行原样打印进会话记录，本身即一条泄露通道，且从未被任何获批工作包点名，禁用属范围收窄。
4. 该门为文本检查型，**在原理上无法完备**：它使获批形态精确、使规避必须蓄意构造，而蓄意构造式规避由公共接收规则「门拒绝即停止条件、不得换工具或改写命令重试」约束。本条不声称该门不可绕过。
5. 常设自测同时纳入允许用例、拒绝用例与「不相关命令不得误拒」用例，由系统测试入口在产品断言之前调用；任一缺失即在产品断言之前 fail-closed。

销案条件：本条与机器闸门提交、§5.5 关闭门通过并交付完整证据后，由所有者裁定是否销案；实施 PASS 不自动销案。
...
- 2026-07-19 v1.3 登记获批脚本参数面授权边界事件；G3-M 增补参数面有限命令门与常设正负例（起草：制包 CC；批准：项目决策人）。
