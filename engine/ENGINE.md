...
The engine adapts to `data/` and `schemas/` — never the reverse (gate G2). No new formulas, variables, units, or models; no algebraic inversion. The F008 low-speed policy is enacted (Stage 5): below 10 mph, F008 results carry the low_speed_ideal_model risk warning; V = 0 remains constraint-blocked. Plausibility ranges are Stage 5 calibrated. Unit-misuse copy belongs to work branch 5.
