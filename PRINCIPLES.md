...
| G6 | P6 | 执行与评审为不同会话／系统；独立复核脚本由评审方维护、实现方不得改动；验收含实现方事先不可见的现场出题环节。执行会话默认禁止启动子代理；任何放开须由所有者通过后续治理变更明确授权，且不得弱化 G3。 |
...
| G10 | P10 | 事件闭环三件套齐备方可销案：事实记录（登记册或私有事件档）＋本文件条目（带日期）＋对应闸门。对应闸门必须进入常设测试入口并含至少一道拒绝用例；只写提示语不算机器闸门。 |
| G3-M | P3/P6/P10 | Claude Code 项目级机器闸门由共享设置与 `validation/system/governance_gate.mjs`共同实施：G3 禁读边界向一切派生角色继承；无点名范围的检索、裸递归检索、后台任务及临时探针仓创建在工具执行前 fail-closed；`Monitor` 与独立 `PowerShell` 工具默认禁用，防止绕过受控 Bash 面；门本身由系统测试入口持续自检。 |

---

### 2026-07-18 · Part 9 执行边界事件

公开事实摘要：执行会话在未完整恢复治理原文的情况下启动了派生角色；其中一次检索越过 G3 边界，派生角色另创建了未获授权的探针／scratch／临时对象。技术发布链最终经独立复核关闭，但两项治理事件在三件套补齐前保持开放；完整事实、未知路径与所有者裁决留在私有事件档。

永久化处理：

1. 新增公共 `CLAUDE.md` 接收闸门，要求执行会话在首次工具调用前恢复原则、当前获批交接件／工作包、角色与五问；固定五项收口不得因追求简短而省略。
2. 共享 Claude Code 设置默认禁用派生角色、`Monitor` 与独立 `PowerShell` 工具，并以 deny 规则和 `PreToolUse` command hook 在工具执行前阻断 G3 边界、裸递归检索、后台任务及自建临时探针仓。
3. `validation/system/governance_gate.mjs`同时作为运行时 hook 与确定性自测模块；`node validation/system/run.js`每次运行先验证共享设置、公共接收规则和正／负例，任一缺失即在产品系统断言之前 fail-closed。
4. 项目级 hook 是可提交、可测试的第一道执行门，不冒充原生 Windows 上不存在的 OS 级隔离；所有者在岗、独立评审和外部写停点继续有效。

销案条件：本条与机器闸门提交并通过独立负例验证后，由所有者分别裁定两项事件是否销案；阶段完成或技术 PASS 不自动销案。

---

修订记录：

- 2026-07-13 v1.0 建档（起草：评审方；批准：项目决策人）。
- 2026-07-18 v1.1 登记 Part 9 执行边界事件；强化 G6/G10，新增 G3-M 机器闸门与销案条件（起草：评审方；批准：项目决策人）。
- 2026-07-18 v1.2 补齐 `Monitor`／`PowerShell` 工具面默认拒绝、hook 覆盖与常设拒绝自测（起草：评审方；批准：项目决策人）。
